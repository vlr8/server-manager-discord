...
use_proxies = True
proxy_list = proxies.read_proxy_file("proxies.txt")
if len(proxy_list) == 0:
	use_proxies = False
...
if not os.path.exists("twitter_sessions.json"):
	...
	for i in range(num_accounts):
		...
		if not os.path.exists("twitter_sessions.json"):
			with open("twitter_sessions.json", "w") as f:
				json.dump([], f, indent=4)
			
			accounts_file = []
		
		else:
			with open("twitter_sessions.json", "r") as f:
